...
def fetch_game_stats(df: pd.DataFrame) -> pd.DataFrame:
    results = []
    game_ids = df['GAME_ID'].unique()[984:]
    print(len(game_ids))
...
def main():
    df = pd.read_parquet('player_game_logs_with_features_2024-25_FINAL.parquet')
    game_df = pd.read_parquet('game_stats_2024-25_FINAL.parquet')
    game_stats = fetch_game_stats(df)
    print(game_stats.head())
    game_df = pd.concat([game_df, game_stats], ignore_index=True)
    game_df.to_parquet('game_stats_2024-25_FINAL.parquet')
